...
class DHR_GAT(BaseModel):
    """
    整体结构编码模块 —— DHR-GAT
    用于替代传统 GAT 获取富含长程依赖结构和复杂关系的嵌入
    """

    def __init__(self, args):
        super(DHR_GAT, self).__init__(args)
        self.num_nodes = len(args.entity2id)
        self.entity_in_dim = args.dim
        self.entity_out_dim = args.dim
        self.num_relation = len(args.relation2id)
        ...
        if args.pre_trained:
            self.entity_embeddings = nn.Parameter(
                torch.from_numpy(pickle.load(open('datasets/' + args.dataset + '/entity2vec.pkl', 'rb'))).float())
            self.relation_embeddings = nn.Parameter(
                torch.from_numpy(pickle.load(open('datasets/' + args.dataset + '/relation2vec.pkl', 'rb'))).float())

        self.final_entity_embeddings = nn.Parameter(torch.randn(self.num_nodes, self.entity_out_dim * self.nheads_GAT))
        self.final_relation_embeddings = nn.Parameter(
            torch.randn(self.num_relation, self.relation_out_dim * self.nheads_GAT))

        # 核心 DHR-GAT 层
        self.spgat = SpDHRGAT(self.num_nodes, self.entity_in_dim, self.entity_out_dim,
                              self.drop_GAT, self.alpha, self.nheads_GAT)
    ...
    def forward(self, adj, train_indices):
        edge_list = adj[0]
        # 若传入的 adj 未包含 edge_type，则构造默认降级策略，通常 adj 结构应由 Data Loader 保证
        edge_type = adj[1] if len(adj) > 1 else torch.zeros(edge_list.shape[1], dtype=torch.long)
        ...
        out_entity, out_relation = self.spgat(self.entity_embeddings, self.relation_embeddings, edge_list, edge_type)
    ...
    def loss_func(self, train_indices, entity_embeddings, relation_embeddings):
        # 损失计算逻辑与原始 GAT 保持一致
        len_pos_triples = int(train_indices.shape[0] / (int(self.neg_num) + 1))
        pos_triples = train_indices[:len_pos_triples].repeat(int(self.neg_num), 1)
        neg_triples = train_indices[len_pos_triples:]

        source_embeds = entity_embeddings[pos_triples[:, 0]]
        relation_embeds = relation_embeddings[pos_triples[:, 1]]
        tail_embeds = entity_embeddings[pos_triples[:, 2]]
        pos_norm = torch.norm(source_embeds + relation_embeds - tail_embeds, p=1, dim=1)

        source_embeds = entity_embeddings[neg_triples[:, 0]]
        relation_embeds = relation_embeddings[neg_triples[:, 1]]
        tail_embeds = entity_embeddings[neg_triples[:, 2]]
        neg_norm = torch.norm(source_embeds + relation_embeds - tail_embeds, p=1, dim=1)
